...
Program::Program(const cl_context _context, const Device& _device, const std::string& _source) :
    m_sources(_source),
    m_device(_device)
{
...
{
    cl_int err = clBuildProgram(m_program, 1, &m_device.m_device, nullptr, nullptr, nullptr);
    if(err == CL_BUILD_PROGRAM_FAILURE)
    {
        size_t infoSize;
        {
            cl_int err = clGetProgramBuildInfo(m_program, m_device.m_device, CL_PROGRAM_BUILD_LOG, 0, nullptr, &infoSize);
            ::exception::checkCLError(err);
        }
        std::string info;
        info.resize(infoSize);
        {
            cl_int err = clGetProgramBuildInfo(m_program, m_device.m_device, CL_PROGRAM_BUILD_LOG, infoSize, &info[0], nullptr);
            ::exception::checkCLError(err);
        }
    ...
    do
    {
        err = clGetProgramBuildInfo(m_program, m_device.m_device, CL_PROGRAM_BUILD_STATUS, sizeof(buildStatus), &buildStatus, nullptr);
        ::exception::checkCLError(err);
